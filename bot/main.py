...
def get_and_send_schedule(bot, chat_id, day_offset=0):
    ...
    weather = get_weather("today") if day_offset == 0 else get_weather("tomorrow")
...
def help(message):
    bot.send_message(
        message.chat.id,
        "Бот выдает погоду и расписание группы ИЦТМС 4-2\n\t"  # noqa E501
...
def week(message):
    ...
    bot.send_message(
        message.chat.id,
        week_parse(result, translate[message.text[1:]]),
